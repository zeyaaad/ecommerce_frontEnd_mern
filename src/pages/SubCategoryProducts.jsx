...
export default function SubCategoryProducts() {
  ...
  let [userWishList,setUserWishList]=useState(null)
    ...
    async function getProducts(){
        setProducts(null);
        try {
            let res=await axios.get(`${Host}/api/v1/ecommerce/filter/subCategory/${slug}`);
...
async function getWishList(){
  ...
  return (
    ...
    <>
        {products.length>0?  
      <>
      <h1> All <span className='important'> {name}</span> Products </h1>
      <hr />
    ...
    <div className="product-details">
      ...
      </>:<h1 className='text-center mt-3' > Not Found products  for {name}  subCategory </h1>}
    </>
    :<LoadingPage/>}
    </div>
  )
